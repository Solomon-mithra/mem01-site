...
- **Zep** ([temporal knowledge graph review](https://baeseokjae.github.io/posts/zep-ai-agent-memory-review-2026/), [provider comparison](https://www.developersdigest.tech/blog/best-ai-agent-memory-providers-2026)): bitemporal knowledge graph, strong on temporal reasoning and enterprise compliance. Their LoCoMo score is publicly contested (mem0's paper scored them 65.99, [Zep's rebuttal](https://blog.getzep.com/lies-damn-lies-statistics-is-mem0-really-sota-in-agent-memory/) measured 75.14 and attributes the gap to implementation errors in mem0's replication). That dispute is exactly why this run ports mem0's own open-sourced prompts and judge verbatim: the number is refutable by rerunning the harness.
